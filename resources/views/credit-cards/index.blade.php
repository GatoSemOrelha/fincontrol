...
@section('content')
<div class="topbar">
    <span class="topbar-title">Cartões de crédito</span>
    @if(auth()->user()->isAdmin())
        <button class="btn btn-primary" onclick="openModal('modal-card')"><i class="ti ti-plus"></i>Novo cartão</button>
    @endif
</div>
    ...
    <div class="grid-3">
        @foreach($cards as $card)
            <div class="card">
                ...
                <div style="font-size:11px;color:var(--color-text-tertiary);margin-bottom:4px">Fatura aberta (RF06)</div>
                ...
                <div style="font-size:11px;color:var(--color-text-tertiary)">{{ $card->pending_count }} lançamentos pendentes</div>
                ...
                @if($card->open_invoice_total > 0 && auth()->user()->isAdmin())
                    <div style="margin-top:12px">
                        <button class="btn btn-sm btn-success" onclick="openModal('modal-pay-{{ $card->id }}')" style="width:100%;justify-content:center">
                            <i class="ti ti-check"></i>Pagar fatura (RF08)
                        </button>
                    </div>
                @endif
            </div>
    ...
    <div class="modal">
        ...
        <form method="POST" action="{{ route('credit-cards.store') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" placeholder="Ex: Visa Empresarial" required>
            </div>
            ...
            <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:16px">
                <button type="button" class="btn" onclick="closeModal('modal-card')">Cancelar</button>
